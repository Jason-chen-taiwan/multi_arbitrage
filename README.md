...
- **Fill Skew 保護**：成交後自動推遠報價，防止單邊行情被連續吃單
...
當訂單成交後，系統會自動將該側報價推遠，防止在單邊行情中被連續吃單：

| 參數 | 說明 | 預設值 |
|------|------|--------|
| 每次成交推遠 | 每次成交後將該側報價推遠多少 bps | 15 bps |
| 最大累積 | skew 累積上限 | 80 bps |
| 衰減半衰期 | skew 的指數衰減半衰期 | 3 秒 |

**運作原理**：

- 當 bid 成交 → bid 報價往下推遠，降低繼續被吃的風險
- 當 ask 成交 → ask 報價往上推遠，降低繼續被吃的風險
- 採用**指數衰減**：`skew = 累積(每次成交 × 0.5^(經過時間/半衰期))`
- 連續成交時 skew 會疊加（最多到上限），沒有成交則自然衰減
- 保護單邊行情，同時不影響正常雙向成交的做市
